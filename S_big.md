Simulation in the symmetric tree $S$
================
Kalle Leppälä

## Simulating the 32 admixture scenarios in the symmetric tree $S$

We simulate 1,000,000 independent allelic patterns in 32 admixture
graphs, see [tools.md](tools.md). The number is motivated by the
commonly used (human) genome-wide significance threshold
$0.5 \times 10^{-8}$, which is based on a Bonferroni correction using an
estimated number of effectively independent variants (Xu et al. 2014).
We use the value $\lambda = 1$, so that a unit of time is about $2N_e$
generations, where $N_e$ is the effective population size. If the
generation time is 10 years and $N_e = 10,000$, a unit of time
corresponds to 200,000 years, as an example.

The underlying species tree is $S = (((1,2),(3,4)),5)$, where
populations 1 and 2, and populations 3 and 4 merge at time point 1,
populations 12 and 34 merge at time point 2, and populations 1234 and 5
merge at time point 3. The probability of admixture is always 10%. Gene
flow between terminal branches (16 scenarios as we are not able to
detect gene flow between 1 and 2 or between 3 and 4) is modelled as an
instantaneous event where a lineage has a chance of being transferred to
another population at time point 0.5. Gene flow from 1234 or 12345 to
one of the branches 1, 2, 3 or 4 (8 scenarios) obviously cannot be
instantaneous and is mediated with a ghost population instead. The ghost
population splits from the terminal branch at time point 0.5 and merges
to population 1234 at time point 2.5 and to population 12345 at time
point 3.5. Finally we have gene flow between 12 and 3 or 4, and gene
flow between 34 and 1 or 2 (8 scenarios). In the underlying tree
described above, gene flow from 12 to 3 could be mediated with a ghost
population, but gene flow from 3 to 12 is impossible. Hence, the merging
times in the underlying tree were changed to 0.75, 1.5, 2.25 and 3 for
the merging of 1 and 2, 3 and 4, 12 and 34, and 1234 and 5,
respectively, and the gene flow event was instantaneus at time point
1.125. The same goes for gene flow between 12 and 4, and a symmetrical
choice for gene flow between 34 and 1 or 2.

Note that in contrast to the $D_\text{FOIL}$-method (Pease and Hahn
2015), we remain agnostic with respect to the temporal order of the
merging events between populations 1 and 2 and populations 3 and 4.
Naturally the events have some true order but it’s not something the
user need to weigh in – particularly because the gene flow direction 34
$\rightarrow$ 1 (or 2) mediated with a ghost population still remains a
possibility even if populations 1 and 2 merge first.

For the expected number of mutations in a unit of time we use the value
$\mu = 0.0001$. This is based on a unit of time representing 200,000
years and the per-year-per-locus mutation rate estimate of
$0.5 \times 10^{-9}$. Finally, we set the resampling parameter
$T_\text{max}$ to 0.0055. The probability of the mutation rate weighted
total length of the gene tree exceeding this value is less than one in a
hundred million: Assume that no coalescence occur before the root, at
which point the total length is $15\mu$. After that, the length is
distributed as
$\mu(5 X_{10} + 4 X_6 + 3 X_3 + 2 X_1) = \mu(X_2 + X_{1.5} + X_1 + X_{0.5})$,
where $X_\alpha \sim \text{exp}(\alpha)$ are independent. The
$(1-10^{-9})$-quantile of this hypoexponentially distributed random
variable is at 0.0040 (call the function
`0.0001*qhypoexp(0.99999999, c(2, 1.5, 1, 0.5))` from the package
`sdprisk`).
...
of the eight preferred $\Delta$-statistics on tree $S$ under the 32
admixture scenarios.
...
Here we plot the figures. Comparing to Table 2 in the main text, we see
that all expectations were identified correctly.
...
![](S_big_files/figure-gfm/plots-1.png)<!-- -->
...
<div id="ref-xu2014estimating" class="csl-entry">

Xu, ChangJiang, Ioanna Tachmazidou, Klaudia Walter, Antonio Ciampi,
Eleftheria Zeggini, Celia MT Greenwood, and UK10K Consortium. 2014.
“Estimating Genome-Wide Significance for Whole-Genome Sequencing
Studies.” *Genetic Epidemiology* 38 (4): 281–90.
